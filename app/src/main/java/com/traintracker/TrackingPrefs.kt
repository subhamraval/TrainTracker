...
object TrackingPrefs {
    ...
    private const val KEY_IS_FIRST_CURR_AVBL = "is_first_curr_avbl"
    ...
    fun getConfig(context: Context): TrackingConfig? {
        ...
        return try {
            Gson().fromJson(json, TrackingConfig::class.java)
        } catch (e: Exception) {
            null
        }
    }
    ...
    fun setTracking(context: Context, isTracking: Boolean) {
        prefs(context).edit().putBoolean(KEY_IS_TRACKING, isTracking).apply()
    }
    ...
    fun setFirstCurrAvbl(context: Context, isFirst: Boolean) {
        prefs(context).edit().putBoolean(KEY_IS_FIRST_CURR_AVBL, isFirst).apply()
    }

    fun isFirstCurrAvbl(context: Context): Boolean =
        prefs(context).getBoolean(KEY_IS_FIRST_CURR_AVBL, true)

    fun saveLastCheckedTime(context: Context, time: String) {
        prefs(context).edit().putString(KEY_LAST_CHECKED_TIME, time).apply()
    }
